...
let testStores = null;
export function setStoreForTests(s) { testStores = s; }
export function store(name = 'analytics') {
  if (testStores) return typeof testStores === 'function' ? testStores(name) : testStores;
  const prod = globalThis.Netlify?.context?.deploy?.context === 'production';
  return prod ? getStore(name) : getDeployStore(name);
}
