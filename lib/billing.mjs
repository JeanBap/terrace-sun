...
export async function ensureSetup(base, force = false) {
  ...
  for (const [id, [name, description]] of Object.entries(PRODUCTS)) {
    try { await stripe('GET', `products/${id}`); }
    catch (e) { if (e.status !== 404) throw e; await stripe('POST', 'products', { id, name, description }); }
  }
  const meters = await stripe('GET', 'billing/meters', { status: 'active', limit: 100 });
  ...
  cfg.meter = meter.id;
  const found = await stripe('GET', 'prices', { active: 'true', limit: 100, lookup_keys: Object.keys(PRICES) });
  for (const p of found.data || []) cfg.prices[p.lookup_key] = p.id;
  for (const [lk, def] of Object.entries(PRICES)) {
    if (cfg.prices[lk]) continue;
  ...
  cfg.portal_config = cur?.portal_config ? (await stripe('POST', `billing_portal/configurations/${cur.portal_config}`, portal)).id : (await stripe('POST', 'billing_portal/configurations', portal)).id;
  ...
  for (const h of same) if (h !== keep) await stripe('DELETE', `webhook_endpoints/${h.id}`);
  if (keep) { await stripe('POST', `webhook_endpoints/${keep.id}`, { enabled_events: WEBHOOK_EVENTS, disabled: 'false' }); cfg.webhook = cur.webhook; }
  else { const h = await stripe('POST', 'webhook_endpoints', { url, enabled_events: WEBHOOK_EVENTS, description: 'Terrace Sun billing' }); cfg.webhook = { id: h.id, secret: h.secret, url }; }
  cfg.ok = true;
