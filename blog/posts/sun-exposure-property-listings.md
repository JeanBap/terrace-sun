...
Parameters: `lat`, `lon` (required), `floor` (0 = ground), `floors` (floors in the building), `height` (assumed height for untagged neighbours), `name`, `year`, `key`. Responses are cached for 24 hours per location and floor. The full specification is in [openapi.json](/openapi.json); plans and keys are on the [pricing page](/listings/#pricing).
