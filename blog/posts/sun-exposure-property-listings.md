---
title: Sun exposure in property listings: how to measure it, show it and sell it
slug: sun-exposure-property-listings
seo_title: Sun exposure in property listings: measure it, show it, sell it
description: How many hours of direct sun does a flat get, on which side, at which floor, in winter? Measure it for any address and show it on listings with a Sun Score.
date: 2026-09-10
updated: 2026-09-10
author: Yanni Papoutsi
keywords: sun exposure real estate, sunlight hours apartment, natural light property listing, sun exposure report, Sun Score, how sunny is this flat, which floor gets the most sun, esposizione solare casa, sun exposure API
---
...
Data note: 37% and 45% of the surrounding built area had real building heights in OpenStreetMap; the rest was assumed to be 15.2 m, the typical height of the tagged buildings nearby. [Open the top-floor report](/report/?lat=41.8799368&lon=12.4768852&floor=5&floors=6&name=Piazza%20Testaccio%2C%20top%20floor).
...
Parameters: `lat`, `lon` (required), `floor` (0 = ground), `floors` (floors in the building), `height` (assumed height for untagged neighbours), `name`, `year`, `key`. Results are cached for 30 days per location and floor, so repeat calls answer in well under a second. A first request for a new location can take a few seconds; if the building data is slow to arrive, the API answers `503` with `retry_after_s` and finishes the calculation in the background. The full specification is in [openapi.json](/openapi.json); plans and keys are on the [pricing page](/listings/#pricing).
...
Machine-readable entry points: [llms.txt](/llms.txt), [llms-full.txt](/llms-full.txt), [openapi.json](/openapi.json) and a [Markdown version of this article](/blog/sun-exposure-property-listings/index.md).
